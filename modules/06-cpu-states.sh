...
log_step "CPU performance pinning (governor / no_turbo / max-freq cap / c-states / memory tunings / per-core freq overrides / per-core poll-mode)"
...
#    Otherwise (AMD pstate, acpi-cpufreq): write scaling_max_freq per CPU,
#    computed from the per-CPU cpuinfo_max_freq.
