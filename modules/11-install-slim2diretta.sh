...
#
# install.sh's full path does NOT install the web UI; ./install.sh --webui
# copies webui/ and runs a small Python server as slim2diretta-webui.service
# on :8081. slim2Diretta's --webui asks its own Y/N, so we don't add a second
# prompt here — we just make sure python3 is present (done above) and run it.
if [[ "${DRY_RUN:-0}" -eq 1 ]]; then
