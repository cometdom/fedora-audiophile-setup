...
if [[ "$_fin_ram_kargs" == *"systemd.volatile=state"* ]]; then
    ...
    if [[ -f "$_fin_ram_dracut_conf" ]]; then
        ...
        _fin_skip "RAM mode: audiophile.overlay=1 in cmdline but dracut conf missing — re-run module 13"
    fi
    ...
    _fin_skip "RAM mode not enabled (module 13 skipped or disabled) — ${_fin_ram_headroom_mb} MiB available if enabled"
fi
