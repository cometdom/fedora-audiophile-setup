...
- Installs a **PREEMPT_RT kernel** — your choice of vanilla `kernel-rt` (official Fedora [`@kernel-vanilla/stable` COPR](https://fedoraproject.org/wiki/Kernel_Vanilla_Repositories), default) or `kernel-cachyos-rt` (CachyOS RT, `bieszczaders/kernel-cachyos` COPR) — and sets it as the default boot entry
