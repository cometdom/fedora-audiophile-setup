...
> **Status: v2.4.5** — Production-ready on Fedora 43/44 (x86_64). All modules implemented and tested.
...
- [x] **v2.4.5** — The install guide now creates a **separate `/home` partition** during Anaconda: Fedora Server's Automatic partitioning (what the walkthrough previously recommended) creates a single root LV, no `/home` LV — meaning Persistent paths' default `/home`-based presets would refuse for anyone following the documented install to the letter, not just Tune OS. Section 5.2 (EN + FR) now walks through Anaconda's **Custom** storage mode instead: auto-create the baseline partitions, shrink `/`, add `/home` as its own mount point via the "+" dialog. Full chain validated end-to-end on real hardware (not a VM): fresh install with the new partitioning → `findmnt` confirms `/` and `/home` on distinct LVs → a plain `git pull` under `/home` survives a RAM-mode overlay reboot that previously wiped it → the Persistent-paths custom-pair check that used to refuse on this exact machine now accepts → the resulting bind-mount survives a further reboot.
