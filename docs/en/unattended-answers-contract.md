...
The RPi sibling ([fedora-rpi-audiophile-setup](https://github.com/cometdom/fedora-rpi-audiophile-setup))
...
`13-ram-mode` here vs `14-ram-mode` there, because RPi inserts
`13-pi-tweaks`), and RPi adds `UA_PI_TWEAKS*` keys with no x86 equivalent.
Consumers targeting both platforms should read both files rather than assume
